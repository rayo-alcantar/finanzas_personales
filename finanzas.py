...
class Finanza:
	...
	def actualizar_ingreso(self, index, nuevo_nombre, nueva_cantidad):
		ingresos = self.leer_ingresos()
		if 0 <= index < len(ingresos):
			ingresos[index]['Nombre del Ingreso'] = nuevo_nombre
			ingresos[index]['Cantidad'] = nueva_cantidad
			df = pd.DataFrame(ingresos)
			df.to_csv(self.archivo_ingresos, index=False)